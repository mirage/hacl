...
let sign priv msg =
  let signature = Cstruct.create 64 in (* SHA512 = 512 bit = 64 byte *)
  ed25519_sign (Cstruct.to_bigarray signature) (checked_buffer priv)
    (Cstruct.to_bigarray msg) (Cstruct.len msg);
  signature

let verify ~pub ~msg ~signature =
  ed25519_verify (Cstruct.to_bigarray pub) (Cstruct.to_bigarray msg)
    (Cstruct.len msg) (Cstruct.to_bigarray signature)
